...
- Recovery is cancel then resubmit at the job level.
...
3a. ON TERMINAL STATE / OOM-LIKE SIGNAL — inspect the bounded Task list, then
   describe each failed or suspect Task for its Attempt chain, exit, and root cause:

   uv run iris --config <CONFIG> task list --job <JOB_ID>
   uv run iris --config <CONFIG> task describe <TASK_ID>

   For an exit-137 postmortem, query `iris.task` stats for peak memory before
   concluding that the cgroup limit was reached.
...
7. RECOVER (CANCEL -> RESUBMIT)
   - If current job is still non-terminal, cancel its exact incarnation first:
