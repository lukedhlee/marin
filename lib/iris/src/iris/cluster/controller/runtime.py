...
from iris.cluster.controller.persistence.reconcile.dispatch import (
...
class _TickInputs:
    ...
    timeout_rows: Sequence[reads.ExecutionTimeoutRow] = ()
...
class ControllerRuntime:
    ...
    def __init__(
        ...
        # the DB. This is the only ``| None`` attribute on ControllerRuntime: it is
    ...
    def _timeout_decisions(
        self, timeout_rows: Sequence[reads.ExecutionTimeoutRow], now_ms: int
    ...
    def run_control_tick(self) -> None:
        ...
        """
        if self.started:
            raise RuntimeError("run_control_tick cannot be used after ControllerRuntime.start")
        self._control_tick(
            woken=True,
